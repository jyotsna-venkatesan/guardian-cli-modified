...
def load_config(config_path: str = "config/guardian.yaml") -> Dict[str, Any]:
    ...
    try:
        # Resolve path relative to package location if it's a relative path
        config_file = Path(config_path)
        if not config_file.is_absolute():
            # Get the guardian-cli directory (parent of utils directory)
            package_root = Path(__file__).parent.parent
            config_file = package_root / config_path
