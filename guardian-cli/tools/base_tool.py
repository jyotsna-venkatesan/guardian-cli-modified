    ...
    async def execute(self, target: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode != 0:
                error_message = error.strip() or output.strip() or f"exit code {process.returncode}"
                self.logger.error(
                    f"Tool {self.tool_name} failed with exit code {process.returncode}: {error_message}"
                )
