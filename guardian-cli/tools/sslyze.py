...
class SSLyzeTool(BaseTool):
    ...
    def get_command(self, target: str, **kwargs) -> List[str]:
        ...
        # Strip protocol from target
        target_no_proto = re.sub(r"^https?://", "", target)

        # Parse target (host:port)
        if ":" in target_no_proto:
            host, port = target_no_proto.rsplit(":", 1)
        else:
            # JSON output
            ...
            # Timeout argument removed (not supported by sslyze CLI)

            # Do not add --quiet if using --json_out
            # Add target as last argument
            command.append(target_no_proto)
            return command
